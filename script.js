...
function analyzeExcelContent(workbook, fileName = '') {
    ...
    // If file contains both 'Band' and '2D Barcode', treat as Transcoding
    const isTranscoding = fileNameLower.includes('transcoding') || (fileNameLower.includes('band') && fileNameLower.includes('2d barcode'));
    ...
    if (isGoldPad) {
        ...
        console.log(`File type: SPLIT (Similar to DE-ANO, extends to column S)`);
    } else if (isDePu) {
        ...
        console.log(`File type: TRANSCODING (New Format) - includes Band + 2D Barcode files`);
    } else {
    ...
    workbook.SheetNames.forEach(sheetName => {
        ...
        if (isGoldPad) {
            ...
            // For Split files, use same metadata extraction as DE-ANO
            metadata = extractMetadataDeAno(jsonData);
        } else if (isDePu) {
        ...
        // 2D BARCODE FILES - Similar to DE-ANO but extends to column U
        if (is2DBarcode) {
            console.log('=== 2D BARCODE File Detected ===');
            ...
            // Process parameters from columns J-U (indices 9-20)
            // Skip columns G, H, I (indices 6, 7, 8)
            const specDataStartCol = 9;  // Column J (index 9)
            const specDataEndCol = 21;   // Column V (index 21) - extended to ensure U is captured
            ...
            if (jsonData.length > dataEndRow && machineSerials.length > 0) {
                ...
                for (let col = specDataStartCol; col < specDataEndCol; col++) {
                    ...
                    // For columns T and U (indices 19-20), handle "Scanner Calibration" merged header
                    // Column T (index 19) = X Scanner Calibration
                    // Column U (index 20) = Y Scanner Calibration
                    const isScannerCalibrationColumn = (col === 19 || col === 20);
                ...
                console.log(`Found ${specHeaders.length} spec headers for 2D BARCODE file`);
                ...
                console.log('2D BARCODE machine data structure:', machineData);
                ...
                Object.keys(machineData).forEach(serial => {
                    ...
                    analysis.push({
                        ...
                        mainHeader: '2D Barcode Calibration Data'
                    });
                });
            }
        ...
        // GOLD PAD FILES - Similar to DE-ANO but extends to column U
        if (isGoldPad) {
            console.log('=== GOLD PAD File Detected ===');
            ...
            if (jsonData.length > dataEndRow && machineSerials.length > 0) {
                ...
                console.log(`Found ${specHeaders.length} spec headers for GOLD PAD file`);
                ...
                console.log('GOLD PAD machine data structure:', machineData);
                ...
                Object.keys(machineData).forEach(serial => {
                    ...
                    analysis.push({
                        ...
                        stats: {},
                        parameters: machine.parameters,
                        mainHeader: 'GOLD PAD Calibration Data'
                    });
                });
            }
        ...
        // SPLIT FILES - Similar to DE-ANO but parameters extend only to column S (index 18)
        if (isSplit) {
            console.log('=== SPLIT File Detected ===');
            ...
            // Process parameters from columns J-S (indices 9-18)
            // Skip columns G, H, I (indices 6, 7, 8)
            const specDataStartCol = 9;  // Column J (index 9)
            const specDataEndCol = 19;   // Column T (index 19) - need to include T to capture X Scanner Calibration at index 18
            ...
            if (jsonData.length > dataEndRow && machineSerials.length > 0) {
                ...
                for (let col = specDataStartCol; col < specDataEndCol; col++) {
                    ...
                    // For column S (index 18), handle "Scanner Calibration" if present
                    // Column S (index 18) = X Scanner Calibration (Split files only have X, not Y)
                    const isScannerCalibrationColumn = (col === 18);
                ...
                console.log(`Found ${specHeaders.length} spec headers for SPLIT file`);
                ...
                console.log('SPLIT machine data structure:', machineData);
                ...
                Object.keys(machineData).forEach(serial => {
                    ...
                    analysis.push({
                        ...
                        stats: {},
                        parameters: machine.parameters,
                        mainHeader: 'SPLIT Calibration Data'
                    });
                });
            }
        ...
        if (isDeAno) {
            ...
            // Skip columns G, H, I (indices 6, 7, 8)
            const specDataStartCol = 9;  // Column J (index 9)
            const specDataEndCol = 20;   // Column U (index 20) - extended to ensure T is captured
            ...
            if (jsonData.length > dataEndRow && machineSerials.length > 0) {
                ...
                for (let col = specDataStartCol; col < specDataEndCol; col++) {
                    ...
                    // Column S (index 18) = X Scanner Calibration
                    // Column T (index 19) = Y Scanner Calibration
                    const isScannerCalibrationColumn = (col === 18 || col === 19);
        ...
        if (isTranscoding) {
            ...
            const specDataEndCol = 16;   // Column Q = index 16 (to ensure P is included)
                ...
                for (let col = specDataStartCol; col <= maxCol; col++) {
                    ...
                    if (headerStr && headerStr.length > 0) {
                        ...
                        if (specLimitCell && !isScannerCalibrationColumn) {
                            ...
                            if (numMatch) {
                                // Found a number in Row 6 - treat as -value to +value (symmetric spec)
                                const value = numMatch[1];
                                if (!lsl) lsl = `-${value}`;
                                if (!usl) usl = value;
                                specLimit = `-${value} to +${value}`;
                                console.log(`Found Scanner calibration spec in Row 6 for col ${col}: ${specLimit} (LSL: ${lsl}, USL: ${usl})`);
                            }
                        }
